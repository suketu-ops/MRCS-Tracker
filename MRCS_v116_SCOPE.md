...
> **v116.1 addendum (2026-06-10):** New Attempt wizard now **regenerates the tracker calendar** — the v116 gap where arming a new attempt left the plan hardcoded to Apr 1–21 2026 with no way to add rows. Added: rest-day chips (Mon–Sun, default Sat+Sun), phase-split inputs (PT rev / eM rev / CSC blitz, must total 100%), live plan preview, and `regenerateCycle(examDate, restDays, phases)` — builds rows today→exam; existing rows keep their actuals (only weights/tag update); past rows untouched; last 2 days auto-REST; final week half-weight. Settings ride `REV_GOALS._attempt.{restDays,phases}` (same rev_goals jsonb — no schema change). New LS keys: `mrcs-attempt-rest-days`, `mrcs-attempt-phases`. Also fixed: home `#top-dte` countdown now refreshes on wizard commit (was boot-only). Verified in 390px preview: 135-day Oct-22 plan (95 study/40 rest), 156 rows, idempotent re-arm (0 new · 135 updated), survives reload, 0 console errors. Next: v117 = design system + Tracker row controls; v118 = Tracker page + home redesign.
